...
VapiGenerator::VapiGenerator(void) : outdir_(), systemc_(false) {}
...
std::string VapiGenerator::getTDExternalDecl(void)
{
    ...
    {
        once = true;
        ret << mTopTypeName << "VRTLmodAPI& gTD = " << mTopTypeName
...
std::vector<std::string> VapiGenerator::prepare_sources(const std::vector<std::string> &sources, bool overwrite)
{
    ...
    for (size_t i = 0; i < sources.size(); ++i)
    {
        ...
        {
            util::logging::log(util::logging::WARNING, std::string("Input source ") + sources[i] +
                                                           std::string(" is not a file. Removed from input set."));
        }
        else
        {
    ...
    if (!fs::is_directory(outdir))
    {
        ...
        if (ret == false)
        {
            util::logging::log(util::logging::ERROR,
                               std::string("Failed to create output directory ") + get_outputDir());
        }
    }
    if (overwrite == false)
    {
