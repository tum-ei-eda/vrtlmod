...
void InjectionRewriter::writeSequentInject(ExtDeclFunc *sequent_function)
{
    ...
    for (auto &compound : sequent_function->mCompounds)
    {
        ...
        util::logging::log(util::logging::INFO, std::string("Number of dominant assignments in compound [") +
                                                    compound->_self() + std::string("]:") +
                                                    boost::lexical_cast<std::string>(tInjectionPoints.size()));
...
void InjectionRewriter::writeSequentInject(const BinaryOperator *op, const Expr *base, const Expr *index)
{ //(const BinaryOperator *op){

    int idx;
    util::logging::log(util::logging::INFO,
                       std::string("op: --- ") + getRewriter().getRewrittenText(op->getSourceRange()).c_str());
...
int InjectionRewriter::registerAssignment(ExtAsgnmnt *a)
{
    if (a)
    {
        ...
        {
            std::stringstream x;
            x << "New Assignment (Id: " << a->get_ID() << ") "
              << "in compound (Id: " << tAssignmentLocation->get_ID() << ") "
...
void InjectionRewriter::run(const clang::ast_matchers::MatchFinder::MatchResult &Result)
{
    if (Result.Nodes.getNodeAs<Decl>("function"))
    {
        ...
        {
            util::logging::log(util::logging::INFO,
                               std::string("Matcher LEAVING sequent body of \n\t") + activeSequentFunc->_self());
    ...
    if (const Decl *f = Result.Nodes.getNodeAs<Decl>("sequent_function"))
    {
        ...
        util::logging::log(util::logging::INFO,
                           std::string("Matcher ENTER sequent body of: \n\t") + activeSequentFunc->_self());
    }
    ...
    else
    {
        ...
        if (const BinaryOperator *op = Result.Nodes.getNodeAs<clang::BinaryOperator>("array_assignment"))
        {
            ...
            if (const ArraySubscriptExpr *ase =
                    Result.Nodes.getNodeAs<clang::ArraySubscriptExpr>("arraySubscriptExprLHS"))
            {
...
void InjectionRewriter::addMatcher(clang::ast_matchers::MatchFinder &finder)
{
    ...
    finder.addMatcher(binaryOperator(isAssignmentOperator(), hasLHS(arraySubscriptExpr().bind("arraySubscriptExprLHS")))
                          .bind("array_assignment"),
                      this);
...
void InjectionRewriter::onEndOfTranslationUnit(void)
{
    if (activeSequentFunc != nullptr and in_sequent)
    {
        util::logging::log(util::logging::WARNING,
                           std::string("Matcher ALREADY LEFT sequent body of active\n\t") + activeSequentFunc->_self());
        util::logging::log(util::logging::WARNING,
                           std::string("Invoke wrap-up and rewrite at the end of source file for \n\t") +
                               activeSequentFunc->_self());
        wrap_up_active_sequent();
    }
}
