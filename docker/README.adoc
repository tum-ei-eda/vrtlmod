...
Build times (excluding pulling of base image) on 16-core mobile machine::

- `vrtlmod-base`: 481.7s

Image sizes::

- `vrtlmod-base`: 735.24 MB
...
----
cd path/to/vrtlmod/clone
docker-compose -f docker/docker-compose.yaml up --build # <-- build and up service
docker-compose -f docker/docker-compose.yaml run vrtlmod-workspace # <-- shell into it
----
