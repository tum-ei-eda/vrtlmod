...
#ifdef VRTLMOD
#include "Vcv32e40s_vrtlmodapi.hpp"
#else
#include "Vcv32e40s.h"
#endif
...
#define INSTR_BRIDGE_TYPE mif2tlm_bridge<32, 32, 4, 12, 2, 3, 5, false, true>
#define DATA_BRIDGE_TYPE mif2tlm_bridge<32, 32, 4, 12, 2, 3, 5, true, false>
#define INSTR_BRIDGE_SIGNALS_TYPE mif_signals<32, 32, 4, 12, 2, 3, 5>
#define DATA_BRIDGE_SIGNALS_TYPE mif_signals<32, 32, 4, 12, 2, 3, 5>
...
int sc_main(int argc, char *argv[])
{
    ...
    Vcv32e40s *core = nullptr;
#ifdef VRTLMOD
    auto gFrame = std::make_unique<Vcv32e40sVRTLmodAPI>("vRTLmod");
    core = &(gFrame->vrtl_);
...
#else
    auto gFrame = std::make_unique<Vcv32e40s>("vRTL");
    core = gFrame.get();
#endif
    ...
    core->instr_reqpar_o(instr_bridge_signals->reqpar);

    core->instr_gnt_i(instr_bridge_signals->gnt);       //< automatically grant on request
    core->instr_gntpar_i(instr_bridge_signals->gntpar); //< automatically grant on request

    core->instr_addr_o(instr_bridge_signals->addr);
    core->instr_achk_o(instr_bridge_signals->achk);

    core->instr_rdata_i(instr_bridge_signals->rdata);
    core->instr_rchk_i(instr_bridge_signals->rchk);

    core->instr_rvalid_i(instr_bridge_signals->rvalid);
    core->instr_rvalidpar_i(instr_bridge_signals->rvalidpar);

    core->instr_memtype_o(instr_bridge_signals->memtype);
    core->instr_prot_o(instr_bridge_signals->prot);
    core->instr_dbg_o(instr_bridge_signals->dbg);
    core->instr_err_i(instr_bridge_signals->err);

    core->data_req_o(data_bridge_signals->req);
    core->data_reqpar_o(data_bridge_signals->reqpar);

    core->data_gnt_i(data_bridge_signals->gnt);       //< automatically grant on request
    core->data_gntpar_i(data_bridge_signals->gntpar); //< automatically grant on request

    core->data_rvalid_i(data_bridge_signals->rvalid);
    core->data_rvalidpar_i(data_bridge_signals->rvalidpar);

    core->data_rdata_i(data_bridge_signals->rdata);
    core->data_rchk_i(data_bridge_signals->rchk);

    core->data_addr_o(data_bridge_signals->addr);
    core->data_achk_o(data_bridge_signals->achk);

    core->data_we_o(data_bridge_signals->we);
    core->data_be_o(data_bridge_signals->be);
    core->data_wdata_o(data_bridge_signals->wdata);

    core->data_memtype_o(data_bridge_signals->memtype);
    core->data_prot_o(data_bridge_signals->prot);
    core->data_dbg_o(data_bridge_signals->dbg);
    core->data_err_i(data_bridge_signals->err);
